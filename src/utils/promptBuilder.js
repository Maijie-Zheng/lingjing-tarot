...

{
  ...
  "shareQuote": "用一句话（不超过20字、不加引号）概括本次解读最戳心的洞察。适合作为分享卡的金句标题，让看到卡片的人想'这句话说的是我吗'然后扫码。",
  "shareNarrative": "用3~4行浓缩整体叙事，体现过去→现在→未来的情绪弧线。语气温柔有回味——不是复述narrative，而是提炼出最有画面感的那条线。约60-90字，供分享卡正文用。"
}
...
4. shareQuote 不超过20字，不加引号——它是一句有洞察力的话，不是问题也不是口号。
5. shareNarrative 要有画面感，让读到的人仿佛看到一张牌、一个意象、一个瞬间。
6. JSON 字符串中的双引号写 \"，换行写 \\n，不要破坏 JSON 结构。确保你的整个回复可以被 JSON.parse() 解析。
